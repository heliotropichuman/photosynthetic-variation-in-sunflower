...
# or use load("results/res.RData") and put in an if statement where if false (the file does not exist) run analysis
...
eqX <- paste("eq",c(1,2,3,4,5,6,8,9,11),sep = "") # call data from all equations at once 
...
##### Plot and Print Results from all Equation Models #####
...
# This takes about 2.25 hours to run #
for(e in 1:9)
{
...
save.image(file = "saved_state_1_25_2024.RData") #saves all work that has been done to this poin
...
# The book strap values are the same for all traits???? Are they the bootstraps for the stephens phylogeny, or is there just one reconstruction, and we're using values from that? 
# phi_Icomp_I20
